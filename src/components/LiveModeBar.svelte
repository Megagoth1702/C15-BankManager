<script lang="ts">
  ...
  import { RELEASES_LATEST_URL } from '../lib/live/localPackLinks';
  import {
    connectLive,
    disconnectLive,
    liveMode,
...
<div class="relative flex items-center gap-1.5" data-live-mode-bar>
  ...
  {#if !socketsOk}
    <a
      class="max-w-[14rem] truncate text-[10px] leading-tight text-c15-accent underline-offset-2 hover:underline"
      href={RELEASES_LATEST_URL}
      target="_blank"
      rel="noopener noreferrer"
      title={blockedReason ?? 'Download Local Live pack'}
    >
      Live needs Local pack →
    </a>
  {/if}
